...
- 正式支持 ARMv7 架构设备，覆盖安装、升级、运行全流程。
- ARMv7 版本基于设备实际能力提供核心功能，不包含 eSIM 相关模块，安装时不会错误下载其他架构的组件。

#### 🏗️ 架构优化

- 优化设备面板渐进加载与分频刷新，避免慢请求阻塞首屏或后台刷新重复叠加。
- Device Service 启动时自动修复本机 Agent 身份，确保 Hub 重启或升级后保持本机设备接入方式稳定。
- OTA 升级严格校验安装包与设备架构、系统库的兼容性，从底层避免装错版本。
- eSIM 功能新增架构级能力判断，不支持的设备不会暴露对应接口和功能，能力边界更清晰。

#### 💫 体验优化

- ARMv7 设备前端自动隐藏不支持的功能入口，误访问旧路由时会自动跳转回 SIM 卡基本信息页，避免展示不可用功能造成困惑。
- OTA 页面自动筛选与设备架构匹配的版本，排除跨架构安装包；架构未知时不展示有风险的版本，更新更省心。

#### 🛡️ 安全与稳定性

- 一键安装脚本优化依赖安装逻辑，缺什么装什么，不用每次都刷新软件源，支持多种安装策略按需选择组件。
- 安装临时目录支持多级兜底，不会因为目录缺失导致安装失败；安装和卸载增加互斥锁，防止同时操作引发异常。
- 安装包经过多重校验，确保文件完整、架构匹配；安装采用过渡替换机制，失败自动回滚，相同版本重复安装不会重复执行替换和重启。
- 卸载时会校验文件路径，清理残留数据，不会误删文件；不会随意重启系统服务，也不会卸载系统自带的依赖包。

#### 📦 构建与发布

- 同步提供 ARMv7、ARM64、x86_64 三种架构的安装包，统一命名规范并附带校验文件，发布更规范。
- 构建流程完善，不同架构针对性适配测试，版本稳定性更有保障。

#### 📚 文档更新
- 补充了 ARMv7 架构的安装、使用说明和能力边界介绍。
- 完善了相关测试文档，参考信息更全面。
