...
- 治理 ModemManager 调试提权 drop-in 优先级，采用最高字典序命名（`zz-simadmin-debug.conf`），根除下游移动固件（如 `mobile-tweaks`）覆盖清空调试模式问题，内置旧版 `99-` 遗留配置自动清理。
...
- 卸载脚本深度清理新旧调试提权配置（`zz-` / `99-simadmin-debug.conf`）及二次 QMI 遗留规则，确保宿主机环境纯净无污染。
